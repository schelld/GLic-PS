...
$ReportFile = "$PSScriptRoot\reports\licenses.csv"
$LogFile    = "$PSScriptRoot\logs\Export-Licenses.log"
$null = New-Item -ItemType Directory -Force "$PSScriptRoot\reports","$PSScriptRoot\logs"

try {
    Start-GlicLog $LogFile 'Export-Licenses starting'
    Get-GlicLicenses | Export-Csv $ReportFile -NoTypeInformation -Force
