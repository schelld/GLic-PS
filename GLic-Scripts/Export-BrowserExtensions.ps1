...
$ReportFile = "$PSScriptRoot\reports\browser-extensions.csv"
$LogFile    = "$PSScriptRoot\logs\Export-BrowserExtensions.log"
$null = New-Item -ItemType Directory -Force "$PSScriptRoot\reports","$PSScriptRoot\logs"

try {
    Start-GlicLog $LogFile 'Export-BrowserExtensions starting'
    Get-GlicBrowserExtensions | Export-Csv $ReportFile -NoTypeInformation -Force
