...
function Get-GlicUsers {
    [PSCustomObject]@{ ReportDate='2026-01-01'; PrimaryEmail='user@example.com'; FullName='Test User'; OrgUnit='/' }
}
'@
...
function Get-GlicUsers { throw 'Simulated failure' }
'@
}

Describe 'Export-Users.ps1 — happy path' {
    BeforeAll {
        ...
        Copy-Item "$PSScriptRoot\..\GLic-Scripts\Export-Users.ps1" $script:Dir
        & $script:Ps -NonInteractive -File (Join-Path $script:Dir 'Export-Users.ps1')
    }
    It 'exits 0' { $LASTEXITCODE | Should -Be 0 }
    It 'creates reports\users.csv' { Test-Path (Join-Path $script:Dir 'reports\users.csv') | Should -BeTrue }
    It 'log contains Completed' {
        Get-Content (Join-Path $script:Dir 'logs\Export-Users.log') -Raw | Should -Match 'Completed'
    }
}

Describe 'Export-Users.ps1 — error path' {
    BeforeAll {
    ...
    It 'log contains ERROR' {
        Get-Content (Join-Path $script:Dir 'logs\Export-Users.log') -Raw | Should -Match 'ERROR'
    }
}
