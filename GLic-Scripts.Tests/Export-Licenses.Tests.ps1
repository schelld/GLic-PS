...
function Get-GlicLicenses {
    [PSCustomObject]@{ ReportDate='2026-01-01'; UserEmail='user@example.com'; SkuName='Business Starter'; SkuId='1010020026' }
}
'@
...
function Get-GlicLicenses { throw 'Simulated failure' }
'@
}

Describe 'Export-Licenses.ps1 — happy path' {
    BeforeAll {
        ...
        Copy-Item "$PSScriptRoot\..\GLic-Scripts\Export-Licenses.ps1" $script:Dir
        & $script:Ps -NonInteractive -File (Join-Path $script:Dir 'Export-Licenses.ps1')
    }
    It 'exits 0' { $LASTEXITCODE | Should -Be 0 }
    It 'creates reports\licenses.csv' { Test-Path (Join-Path $script:Dir 'reports\licenses.csv') | Should -BeTrue }
    It 'log contains Completed' { Get-Content (Join-Path $script:Dir 'logs\Export-Licenses.log') -Raw | Should -Match 'Completed' }
}

Describe 'Export-Licenses.ps1 — error path' {
    BeforeAll {
    ...
    It 'log contains ERROR' { Get-Content (Join-Path $script:Dir 'logs\Export-Licenses.log') -Raw | Should -Match 'ERROR' }
}
