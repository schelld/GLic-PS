...
function Get-GlicManagedBrowsers { [PSCustomObject]@{ ReportDate='2026-01-01'; ProfileId='p1'; BrowserVersion='120.0' } }
'@
    $script:ErrorRuntime = @'
$script:GlicModuleRoot = $PSScriptRoot; $script:GlicSession = $null
$script:_glicToken = $null; $script:_glicTokenExpiry = [datetime]::MinValue; $script:GlicScopes = @()
function Start-GlicLog { param([string]$Path,[string]$Message)
    $null = New-Item -ItemType Directory -Force (Split-Path $Path) -ErrorAction SilentlyContinue
    "$([datetime]::UtcNow.ToString('yyyy-MM-dd HH:mm:ss'))Z  $Message" | Add-Content -LiteralPath $Path }
function Get-GlicManagedBrowsers { throw 'Simulated failure' }
'@
}
Describe 'Export-ManagedBrowsers.ps1 — happy path' {
    BeforeAll {
        ...
        Copy-Item "$PSScriptRoot\..\GLic-Scripts\Export-ManagedBrowsers.ps1" $script:Dir
        & $script:Ps -NonInteractive -File (Join-Path $script:Dir 'Export-ManagedBrowsers.ps1')
    }
    It 'exits 0' { $LASTEXITCODE | Should -Be 0 }
    It 'creates reports\managed-browsers.csv' { Test-Path (Join-Path $script:Dir 'reports\managed-browsers.csv') | Should -BeTrue }
    It 'log contains Completed' { Get-Content (Join-Path $script:Dir 'logs\Export-ManagedBrowsers.log') -Raw | Should -Match 'Completed' }
}
Describe 'Export-ManagedBrowsers.ps1 — error path' {
    BeforeAll {
    ...
    It 'log contains ERROR' { Get-Content (Join-Path $script:Dir 'logs\Export-ManagedBrowsers.log') -Raw | Should -Match 'ERROR' }
}
