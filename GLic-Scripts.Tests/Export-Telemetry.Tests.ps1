...
function Get-GlicTelemetry {
    [PSCustomObject]@{ ReportDate='2026-01-01'; DeviceId='d1'; SerialNumber='S1'; Model='CB'; OsVersion='120.0' }
}
'@
...
function Get-GlicTelemetry { throw 'Simulated failure' }
'@
}

Describe 'Export-Telemetry.ps1 — happy path' {
    BeforeAll {
        ...
        Copy-Item "$PSScriptRoot\..\GLic-Scripts\Export-Telemetry.ps1" $script:Dir
        & $script:Ps -NonInteractive -File (Join-Path $script:Dir 'Export-Telemetry.ps1')
    }
    It 'exits 0' { $LASTEXITCODE | Should -Be 0 }
    It 'creates reports\telemetry.csv' { Test-Path (Join-Path $script:Dir 'reports\telemetry.csv') | Should -BeTrue }
    It 'log contains Completed' {
        Get-Content (Join-Path $script:Dir 'logs\Export-Telemetry.log') -Raw | Should -Match 'Completed'
    }
}

Describe 'Export-Telemetry.ps1 — error path' {
    BeforeAll {
    ...
    It 'log contains ERROR' {
        Get-Content (Join-Path $script:Dir 'logs\Export-Telemetry.log') -Raw | Should -Match 'ERROR'
    }
}
